...
const CarouselMainContainer = forwardRef<
  HTMLDivElement,
  { parentClassName?: string } & React.HTMLAttributes<HTMLDivElement>
>(({ className, parentClassName = "", dir, children, ...props }, ref) => {
...
const SliderThumbItem = forwardRef<
  HTMLDivElement,
  {
    ...
    inactiveParentClassName?: string;
    customChildren?: (props: { isSlideActive: boolean }) => React.ReactNode;
  } & React.HTMLAttributes<HTMLDivElement>
>(
  (
    {
      ...
      index,
      parentClassName = "",
      activeParentClassName = "!opacity-100",
      inactiveParentClassName = "opacity-40",
      customChildren,
      children,
      ...props
    },
    ...
    return (
      <div
        ...
        className={cn(
          "flex min-w-0 shrink-0 grow-0 basis-1/3 bg-background p-1 transition-all cursor-pointer",
          `${orientation === "vertical" ? "pb-1" : "pr-1"}`,
          `${isSlideActive ? activeParentClassName : inactiveParentClassName}`,
          parentClassName
        )}
      >
        <div
          className={cn(
            "relative aspect-square h-20 w-full rounded-md transition-opacity",
            className
          )}
        >
